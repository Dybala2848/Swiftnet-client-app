...
// 4. creates a virtual tunnel Iface struct for that interface
func setupIfs(ifs []string) []*Iface {
	...
	for i, v := range ifs {
		...
		fd, err := socks.CreateDeviceBoundUDPSocket(ip, uint16(CLIENT_PORT+i), v)
		if err != nil {
			log.Fatal(err)
		}
		...
		udpaddr, err := net.ResolveUDPAddr("udp", fmt.Sprintf("%s:%d", ip, CLIENT_PORT))
		if err != nil {
			log.Fatal(err)
		}
